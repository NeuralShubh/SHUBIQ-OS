...
const PAGE_NAMES={dashboard:'Dashboard',projects:'Projects',clients:'Clients',invoices:'Documents',expenses:'Expenses',pnl:'P&L Report',products:'Products',subscriptions:'Subscriptions','labs-analytics':'Labs Analytics',documents:'Document Center',settings:'Settings'};
function nav(id){
...
function quickInvoice(pid){const p=DB.projects.find(x=>x.id===pid);if(!p)return;nav('invoices');openNewDoc('invoice');setTimeout(()=>{setSelect('doc-client',p.client);document.getElementById('doc-subject').value=p.name;},100);}

function renderClients(){
  const colors=['#3684DB','#22C55E','#F59E0B','#8B5CF6','#EF4444','#06B6D4','#EC4899','#F97316'];
  document.getElementById('clients-grid').innerHTML=DB.clients.map((c,i)=>`<div class="card" style="cursor:pointer" onclick="editClientInline('${c.id}')"><div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:14px"><div style="width:40px;height:40px;border-radius:10px;background:${colors[i%colors.length]}22;border:1px solid ${colors[i%colors.length]}33;display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-weight:700;font-size:14px;color:${colors[i%colors.length]};flex-shrink:0">${c.name.substring(0,2).toUpperCase()}</div><div style="flex:1;min-width:0"><div style="font-family:var(--font-display);font-size:0.9rem;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${c.name}</div><div style="font-size:0.75rem;color:var(--text4)">${c.contact||''}</div></div><span class="badge ${c.status==='Active'?'badge-green':'badge-gray'} no-dot" style="font-size:0.65rem">${c.status}</span></div><div class="divider" style="margin:12px 0"></div><div style="display:grid;gap:6px">${c.email?`<div style="display:flex;align-items:center;gap:8px;font-size:0.78rem"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:13px;height:13px;color:var(--text4);flex-shrink:0"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg><a href="mailto:${c.email}" style="color:var(--accent);text-decoration:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" onclick="event.stopPropagation()">${c.email}</a></div>`:''}${c.phone?`<div style="display:flex;align-items:center;gap:8px;font-size:0.78rem;color:var(--text3)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:13px;height:13px;color:var(--text4);flex-shrink:0"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 8.8 19.79 19.79 0 01.22 3.18 2 2 0 012.2 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 8.07a16 16 0 006.29 6.29l.63-.63a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>${c.phone}</div>`:''}${c.address?`<div style="font-size:0.75rem;color:var(--text4);margin-top:4px">${c.address.substring(0,60)}</div>`:''}</div></div>`).join('')||'<div style="grid-column:1/-1"><div class="empty-state"><h3>No Clients Yet</h3><p>Add your first client</p></div></div>';
}
function editClientInline(id){const c=DB.clients.find(x=>x.id===id);if(!c)return;document.getElementById('c-name').value=c.name;document.getElementById('c-contact').value=c.contact||'';document.getElementById('c-email').value=c.email||'';document.getElementById('c-phone').value=c.phone||'';setSelect('c-industry',c.industry||'Technology');setSelect('c-status',c.status||'Active');document.getElementById('c-address').value=c.address||'';document.getElementById('c-gst').value=c.gst||'';document.getElementById('c-website').value=c.website||'';const modal=document.getElementById('addClientModal');modal.querySelector('.modal-title').textContent='Edit Client';const btn=modal.querySelector('.btn-primary');btn.textContent='Update Client';btn.setAttribute('onclick',`updateClientById('${id}')`);openModal('addClientModal');}
...
function resetClientModal(){const modal=document.getElementById('addClientModal');modal.querySelector('.modal-title').textContent='Add Client';const btn=modal.querySelector('.btn-primary');btn.textContent='Add Client';btn.setAttribute('onclick','saveClient()');}
...
async function saveDocument(){const type=document.getElementById('doc-type').value;const client=document.getElementById('doc-client').value;if(!client){showToast('Please select a client','error');return;}const items=getLineItems();if(items.length===0){showToast('Add at least one line item','error');return;}const sub=items.reduce((s,i)=>s+(i.qty*i.rate),0);const tp=parseFloat(document.getElementById('doc-tax').value)||0;const ta=sub*tp/100;const tot=sub+ta;const payload={type,client,subject:document.getElementById('doc-subject').value,items,subtotal:sub,tax:tp,taxAmt:ta,total:tot,currency:document.getElementById('doc-currency').value,date:document.getElementById('doc-date').value,due:document.getElementById('doc-due').value,status:type==='invoice'?'Pending':type==='quotation'?'Pending':'Draft',notes:document.getElementById('doc-notes').value};try{const res=await apiFetch('/api/documents',{method:'POST',body:JSON.stringify(payload)});DB.documents.unshift(res);const pref=type==='invoice'?'INV':type==='quotation'?'QUO':'PRO';DB.counters[pref]=(DB.counters[pref]||0)+1;closeModal('newDocModal');renderDocs();renderDocStats();updateNavBadges();showToast(pref+' created: '+res.num,'success');}catch(e){showToast(e.message,'error');}}
...
let expFilter='',expCatFilter='',expCharts={};
function renderExpStats(){const total=DB.expenses.reduce((s,e)=>s+e.amount,0);const byCat={};DB.expenses.forEach(e=>{byCat[e.category]=(byCat[e.category]||0)+e.amount;});const topCat=Object.entries(byCat).sort((a,b)=>b[1]-a[1])[0];const thisMonth=DB.expenses.filter(e=>e.date&&e.date.startsWith(new Date().toISOString().substring(0,7))).reduce((s,e)=>s+e.amount,0);document.getElementById('exp-stats').innerHTML=`<div class="stat"><div class="stat-label">Total Expenses</div><div class="stat-value red">${fmt(total)}</div><div class="stat-change neutral">${DB.expenses.length} entries</div><div class="stat-icon red"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></div></div><div class="stat"><div class="stat-label">This Month</div><div class="stat-value">${fmt(thisMonth)}</div><div class="stat-change neutral">Current month</div><div class="stat-icon amber"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/></svg></div></div><div class="stat"><div class="stat-label">Top Category</div><div class="stat-value" style="font-size:1.2rem">${topCat?topCat[0]:'—'}</div><div class="stat-change neutral">${topCat?fmt(topCat[1]):''}</div><div class="stat-icon blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/></svg></div></div><div class="stat"><div class="stat-label">Avg Monthly</div><div class="stat-value">${fmt(total/12)}</div><div class="stat-change neutral">Per month</div><div class="stat-icon purple"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div></div>`;}
function renderExpenses(){const catColors={'Software':'#3684DB','Marketing':'#F59E0B','Payroll':'#22C55E','Office':'#8B5CF6','Infrastructure':'#EF4444','Travel':'#06B6D4','Misc':'#758BA5'};const byCat={};DB.expenses.forEach(e=>{byCat[e.category]=(byCat[e.category]||0)+e.amount;});if(expCharts.cat){expCharts.cat.destroy();expCharts.cat=null;}const cc=document.getElementById('expCatChart');if(cc){const cats=Object.entries(byCat).sort((a,b)=>b[1]-a[1]);if(cats.length){expCharts.cat=new Chart(cc,{type:'doughnut',data:{labels:cats.map(c=>c[0]),datasets:[{data:cats.map(c=>c[1]),backgroundColor:cats.map(c=>catColors[c[0]]||'#758BA5'),borderColor:'#0D2237',borderWidth:2,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'65%',plugins:{legend:{display:false}}}});const total=Object.values(byCat).reduce((s,v)=>s+v,0);document.getElementById('exp-cat-legend').innerHTML=cats.map(c=>`<div class="qm-row"><div style="display:flex;align-items:center;gap:6px"><div class="dot" style="background:${catColors[c[0]]||'#758BA5'}"></div><div class="qm-label">${c[0]}</div></div><div><div class="qm-value">${fmt(c[1])}</div><div style="font-size:0.68rem;color:var(--text4)">${total>0?Math.round(c[1]/total*100):0}%</div></div></div>`).join('');}}if(expCharts.trend){expCharts.trend.destroy();expCharts.trend=null;}const tc=document.getElementById('expTrendChart');const sym=cs(DB.settings.currency||'INR');if(tc){const months=['Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar'];const data=buildMonthlyData(DB.expenses,'amount','date');expCharts.trend=new Chart(tc,{type:'line',data:{labels:months,datasets:[{data,borderColor:'#EF4444',backgroundColor:'rgba(239,68,68,0.06)',fill:true,tension:0.4,pointRadius:3,pointBackgroundColor:'#EF4444'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#758BA5',font:{size:10}},grid:{color:'rgba(54,132,219,0.05)'}},y:{ticks:{color:'#758BA5',font:{size:10},callback:v=>sym+(v/1000)+'K'},grid:{color:'rgba(54,132,219,0.05)'}}}}});}let data=DB.expenses;if(expFilter)data=data.filter(e=>e.description.toLowerCase().includes(expFilter)||e.vendor.toLowerCase().includes(expFilter));if(expCatFilter)data=data.filter(e=>e.category===expCatFilter);data=[...data].sort((a,b)=>new Date(b.date)-new Date(a.date));document.getElementById('expenses-tbody').innerHTML=data.map(e=>`<tr><td><div class="td-main">${e.description}</div>${e.notes?`<div class="td-sub">${e.notes}</div>`:''}</td><td><span class="badge no-dot" style="background:${catColors[e.category]||'#758BA5'}18;color:${catColors[e.category]||'#758BA5'};border:1px solid ${catColors[e.category]||'#758BA5'}30">${e.category}</span></td><td style="font-weight:600;color:var(--red)">${fmtFull(e.amount,e.currency)}</td><td><span class="badge badge-gray no-dot" style="font-size:0.7rem">${e.currency}</span></td><td style="color:var(--text3);font-size:0.8rem">${formatDate(e.date)}</td><td style="color:var(--text3)">${e.vendor||'—'}</td><td><button class="btn btn-ghost btn-sm" onclick="deleteExpense('${e.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg></button></td></tr>`).join('')||'<tr><td colspan="7"><div class="empty-state"><h3>No Expenses</h3><p>Add your first expense</p></div></td></tr>';}
...
async function saveExpense(){const description=document.getElementById('ex-desc').value.trim();if(!description){showToast('Description required','error');return;}const payload={description,category:document.getElementById('ex-cat').value,vendor:document.getElementById('ex-vendor').value,amount:parseFloat(document.getElementById('ex-amount').value)||0,currency:document.getElementById('ex-currency').value,date:document.getElementById('ex-date').value,notes:document.getElementById('ex-notes').value};try{const res=await apiFetch('/api/expenses',{method:'POST',body:JSON.stringify(payload)});DB.expenses.unshift(res);closeModal('addExpenseModal');renderExpenses();renderExpStats();showToast('Expense added','success');}catch(e){showToast(e.message,'error');}}
function deleteExpense(id){confirmAction('Delete this expense?',async()=>{try{await apiFetch('/api/expenses/'+id,{method:'DELETE'});DB.expenses=DB.expenses.filter(e=>e.id!==id);renderExpenses();renderExpStats();showToast('Expense deleted','info');}catch(e){showToast(e.message,'error');}});}

let pnlCharts={};
let pnlMonth = new Date().getMonth();
let pnlYear = new Date().getFullYear();

function setPnlLabel(){
  const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const lbl=document.getElementById('pnl-month-label');
  if(lbl) lbl.textContent = `${months[pnlMonth]} ${pnlYear}`;
}
...
function prevPnlMonth(){
  pnlMonth--;
  if(pnlMonth<0){pnlMonth=11;pnlYear--;}
  renderPnL();
}
function nextPnlMonth(){
  pnlMonth++;
...
function renderPnL(){
  ...
  const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const monthLabel=months[pnlMonth];

  const inMonth = d => {
    const dt=new Date(d);
    return !isNaN(dt) && dt.getFullYear()===pnlYear && dt.getMonth()===pnlMonth;
  };

  const sm = [DB.documents.filter(d=>d.type==='invoice'&&d.status==='Paid'&&inMonth(d.date)).reduce((s,d)=>s+d.total,0)];
  const lm = [DB.subscriptions.filter(s=>inMonth(s.date)).reduce((s,x)=>s+x.amount,0)];
  const em = [DB.expenses.filter(e=>inMonth(e.date)).reduce((s,e)=>s+e.amount,0)];
  ...
  document.getElementById('pnl-stats').innerHTML=`<div class="stat"><div class="stat-label">Total Revenue</div><div class="stat-value">${fmt(tr)}</div><div class="stat-change up">${monthLabel} ${pnlYear}</div><div class="stat-icon blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div></div><div class="stat"><div class="stat-label">Total Expenses</div><div class="stat-value red">${fmt(te)}</div><div class="stat-change down">Outflows</div><div class="stat-icon red"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12V7H5a2 2 0 010-4h14v4"/><path d="M3 5v14a2 2 0 002 2h16v-5"/></svg></div></div><div class="stat"><div class="stat-label">Net Profit</div><div class="stat-value green">${fmt(np)}</div><div class="stat-change up">After expenses</div><div class="stat-icon green"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></div></div><div class="stat"><div class="stat-label">Profit Margin</div><div class="stat-value ${mg>30?'green':mg>15?'amber':'red'}">${mg}%</div><div class="stat-change ${mg>30?'up':mg>15?'neutral':'down'}">Net margin</div><div class="stat-icon ${mg>30?'green':'amber'}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg></div></div>`;

  if(pnlCharts.bar){pnlCharts.bar.destroy();pnlCharts.bar=null;}
  const bc=document.getElementById('pnlBarChart');
  if(bc){pnlCharts.bar=new Chart(bc,{type:'bar',data:{labels:[monthLabel],datasets:[{label:'Revenue',data:ti,backgroundColor:'rgba(34,197,94,0.55)',borderColor:'#22C55E',borderWidth:1,borderRadius:4},{label:'Expenses',data:em,backgroundColor:'rgba(239,68,68,0.45)',borderColor:'#EF4444',borderWidth:1,borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#758BA5',font:{size:10}},grid:{color:'rgba(54,132,219,0.05)'}},y:{ticks:{color:'#758BA5',font:{size:10},callback:v=>sym+(v/1000)+'K'},grid:{color:'rgba(54,132,219,0.05)'}}}}});}

  if(pnlCharts.profit){pnlCharts.profit.destroy();pnlCharts.profit=null;}
  const pc=document.getElementById('pnlProfitChart');
  if(pc){pnlCharts.profit=new Chart(pc,{type:'line',data:{labels:[monthLabel],datasets:[{data:nm,borderColor:'#22C55E',backgroundColor:'rgba(34,197,94,0.07)',fill:true,tension:0.4,pointRadius:4,pointBackgroundColor:'#22C55E'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#758BA5',font:{size:10}},grid:{color:'rgba(54,132,219,0.05)'}},y:{ticks:{color:'#758BA5',font:{size:10},callback:v=>sym+(v/1000)+'K'},grid:{color:'rgba(54,132,219,0.05)'}}}}});}

  document.getElementById('pnl-table').innerHTML=`<div style="overflow-x:auto"><table style="background:transparent"><thead><tr><th>Category</th><th style="text-align:right">${monthLabel}</th><th style="text-align:right">Total</th></tr></thead><tbody><tr style="background:rgba(34,197,94,0.04)"><td style="font-weight:600;color:var(--green)">Studio Revenue</td><td style="text-align:right;color:var(--green);font-size:0.78rem">${sym}${(sm[0]/1000).toFixed(0)}K</td><td style="text-align:right;font-weight:700;color:var(--green)">${fmt(sm.reduce((s,v)=>s+v,0))}</td></tr><tr style="background:rgba(245,158,11,0.04)"><td style="font-weight:600;color:var(--amber)">Labs Revenue</td><td style="text-align:right;color:var(--amber);font-size:0.78rem">${sym}${(lm[0]/1000).toFixed(0)}K</td><td style="text-align:right;font-weight:700;color:var(--amber)">${fmt(lm.reduce((s,v)=>s+v,0))}</td></tr><tr style="background:rgba(54,132,219,0.05)"><td style="font-weight:700;color:var(--text)">Total Revenue</td><td style="text-align:right;font-weight:600;font-size:0.78rem">${sym}${(ti[0]/1000).toFixed(0)}K</td><td style="text-align:right;font-weight:700">${fmt(tr)}</td></tr><tr style="background:rgba(239,68,68,0.04)"><td style="font-weight:600;color:var(--red)">Total Expenses</td><td style="text-align:right;color:var(--red);font-size:0.78rem">${sym}${(em[0]/1000).toFixed(0)}K</td><td style="text-align:right;font-weight:700;color:var(--red)">${fmt(te)}</td></tr><tr style="background:rgba(34,197,94,0.08);border-top:2px solid rgba(54,132,219,0.2)"><td style="font-weight:700;color:var(--green);font-size:0.92rem">Net Profit</td><td style="text-align:right;font-weight:700;color:${nm[0]>=0?'var(--green)':'var(--red)'};font-size:0.78rem">${sym}${(nm[0]/1000).toFixed(0)}K</td><td style="text-align:right;font-weight:700;color:var(--green);font-size:0.95rem">${fmt(np)}</td></tr></tbody></table></div>`;
}
...
function loadSettings(){const s=DB.settings;const map={'biz-name':'bizName','owner-name':'ownerName','email':'email','phone':'phone','address':'address','gst':'gst','website':'website','bank-name':'bankName','bank-holder':'bankHolder','bank-acc':'bankAcc','bank-ifsc':'bankIfsc','upi':'upi','inv-notes':'invNotes'};Object.entries(map).forEach(([f,k])=>{const el=document.getElementById('s-'+f);if(el)el.value=s[k]||'';});setSelect('s-currency',s.currency||'INR');document.getElementById('s-tax').value=s.tax||18;document.getElementById('s-terms').value=s.terms||30;setSelect('s-fy',s.fyStart||'April');}
async function saveSettings(){DB.settings={...DB.settings,bizName:document.getElementById('s-biz-name').value,ownerName:document.getElementById('s-owner-name').value,email:document.getElementById('s-email').value,phone:document.getElementById('s-phone').value,address:document.getElementById('s-address').value,gst:document.getElementById('s-gst').value,website:document.getElementById('s-website').value,bankName:document.getElementById('s-bank-name').value,bankHolder:document.getElementById('s-bank-holder').value,bankAcc:document.getElementById('s-bank-acc').value,bankIfsc:document.getElementById('s-bank-ifsc').value,upi:document.getElementById('s-upi').value,currency:document.getElementById('s-currency').value,tax:parseFloat(document.getElementById('s-tax').value)||18,terms:parseInt(document.getElementById('s-terms').value)||30,fyStart:document.getElementById('s-fy').value,invNotes:document.getElementById('s-inv-notes').value};try{await apiFetch('/api/settings',{method:'PUT',body:JSON.stringify(DB.settings)});document.getElementById('user-name-sb').textContent=DB.settings.ownerName||'Admin';document.getElementById('user-avatar-sb').textContent=(DB.settings.ownerName||'AD').substring(0,2).toUpperCase();showToast('Settings saved successfully','success');}catch(e){showToast(e.message,'error');}}
