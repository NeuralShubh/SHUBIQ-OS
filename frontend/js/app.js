...
function renderDashboard(){
  ...
  if(sc){dashCharts.split=new Chart(sc,{type:'doughnut',data:{datasets:[{data:[si,li],backgroundColor:['#3684DB','#F59E0B'],borderColor:'#0D2237',borderWidth:1.5,hoverOffset:5}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:{display:false}}}});}
  ...
  const outs=DB.documents.filter(d=>d.type==='invoice'&&d.status!=='Paid');
  document.getElementById('dash-outstanding').innerHTML=outs.map(d=>{const cl=DB.clients.find(c=>c.id===d.client);const ov=new Date(d.due)<new Date();const remain=Math.max(d.total-(d.paidAmount||0),0);const label=ov?'Overdue':(d.status==='Partially Paid'?'Partial':'Pending');return`<div class="hover-row" onclick="nav('invoices')"><div><div style="font-size:0.83rem;font-weight:600;color:var(--text)">${d.num}</div><div style="font-size:0.72rem;color:var(--text4)">${cl?cl.name:'-'}</div></div><div style="text-align:right"><div style="font-size:0.88rem;font-weight:600;color:var(--amber)">${fmtFull(remain,d.currency)}</div><div class="badge badge-${ov?'red':'amber'} no-dot" style="font-size:0.68rem;margin-top:3px">${label}</div></div></div>`;}).join('')||'<div class="empty-state" style="padding:30px 20px"><p>No outstanding payments</p></div>';
...
function previewDocument(){
  ...
  const tp=parseFloat(document.getElementById('doc-tax').value)||0;
  const ta=subTotal*tp/100;
  const totalVal=subTotal+ta;
  const d={type,num:document.getElementById('doc-num').value,client,subject:document.getElementById('doc-subject').value,items,subtotal:subTotal,tax:tp,taxAmt:ta,total:totalVal,currency:document.getElementById('doc-currency').value,date:document.getElementById('doc-date').value,due:document.getElementById('doc-due').value,notes:document.getElementById('doc-notes').value,status:'Preview'};
  closeModal('newDocModal');
  document.getElementById('doc-preview-content').innerHTML=buildDocHTML(d);
  openModal('docPreviewModal');
}

function buildDocHTML(d){
  const s=DB.settings;
  const cl=DB.clients.find(c=>c.id===d.client);
  const sym=cs(d.currency);
  const tl=d.type.charAt(0).toUpperCase()+d.type.slice(1);
  const sc=d.status==='Paid'?'#10b981':d.status==='Overdue'?'#ef4444':'#f59e0b';
  const sub=parseFloat(d.subtotal||0).toLocaleString('en-IN',{minimumFractionDigits:2});
  const tax=parseFloat(d.taxAmt||0).toLocaleString('en-IN',{minimumFractionDigits:2});
  const tot=parseFloat(d.total||0).toLocaleString('en-IN',{minimumFractionDigits:2});

  return `
  <div class="premium-doc" id="printable-doc">
    ...
    <div class="doc-header">
      <div class="doc-brand-block">
        ...
        <div class="doc-biz-meta">
          ...
          ${s.address?`<div style="max-width:240px">${s.address}</div>`:''}
          ${s.gst?`<div class="doc-gst-label">GST: ${s.gst}</div>`:''}
        </div>
      </div>
      <div class="doc-meta-block">
        <div class="doc-type-label">${tl.toUpperCase()}</div>
        <div class="doc-num-val">${d.num}</div>
        <div class="doc-date-grid">
          <div class="date-item"><span>DATE</span> ${formatDateFull(d.date)}</div>
          ${d.due?`<div class="date-item"><span>DUE</span> ${formatDateFull(d.due)}</div>`:''}
        </div>
        <div class="doc-status-badge" style="background:${sc}15;color:${sc};border-color:${sc}30">${d.status}</div>
      </div>
    </div>
    ...
    <div class="doc-info-section">
      ...
      <div class="info-group">
        <div class="info-label">SUBJECT / PROJECT</div>
        <div class="info-value-main">${d.subject||'-'}</div>
      </div>
    </div>
    ...
    <table class="doc-table">
      ...
      <tbody>
        ${(d.items||[]).map(i=>`
          <tr>
            <td class="td-desc">${i.desc}</td>
            <td style="text-align:center">${i.qty}</td>
            <td style="text-align:right">${sym}${parseFloat(i.rate).toLocaleString('en-IN',{minimumFractionDigits:2})}</td>
            <td class="td-amt-bold">${sym}${(i.qty*i.rate).toLocaleString('en-IN',{minimumFractionDigits:2})}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
    ...
    <div class="doc-footer-flex">
      <div class="doc-notes-block">
        ${s.bankName && d.type==='invoice' ? `
          <div class="payment-details">
            <div class="pay-title">PAYMENT DETAILS</div>
            <div class="pay-grid">
              <div><span class="pay-label">Bank:</span> ${s.bankName}</div>
              <div><span class="pay-label">Account No:</span> ${s.bankAcc}</div>
              <div><span class="pay-label">Account Name:</span> ${s.bankHolder}</div>
              <div><span class="pay-label">IFSC:</span> ${s.bankIfsc}</div>
              ${s.upi?`<div style="grid-column:1/-1"><span class="pay-label">UPI ID:</span> ${s.upi}</div>`:''}
            </div>
          </div>
        ` : ''}
        ${d.notes ? `<div class="doc-notes-txt"><div class="pay-title">NOTES</div>${d.notes}</div>` : ''}
      </div>
      <div class="doc-totals-block">
        <div class="total-row"><span>Subtotal</span><span>${sym}${sub}</span></div>
        ${d.tax>0?`<div class="total-row"><span>Tax (${d.tax}%)</span><span>${sym}${tax}</span></div>`:''}
        <div class="total-row grand">
          <span>Amount Due</span>
          <span class="grand-val">${sym}${tot}</span>
        </div>
      </div>
    </div>

    <div class="doc-final-footer">
      <div>Generated via <b>shubiq.com</b></div>
...
function printDoc(){
  const c=document.getElementById('printable-doc');
  if(!c)return;
...
function renderIncomeStats(){
  const invs=DB.documents.filter(d=>d.type==='invoice'&&(d.status==='Paid'||d.status==='Partially Paid'));
  const total=invs.reduce((s,d)=>s+(d.status==='Paid'?d.total:(d.paidAmount||0)),0);
  const thisMonth=invs.filter(d=>d.date&&d.date.startsWith(new Date().toISOString().substring(0,7))).reduce((s,d)=>s+(d.status==='Paid'?d.total:(d.paidAmount||0)),0);
  const byClient={};invs.forEach(d=>{const c=getClientName(d.client)||'Unknown';byClient[c]=(byClient[c]||0)+(d.status==='Paid'?d.total:(d.paidAmount||0));});
...
function renderExpenses(){
  ...
  if(cc){
    ...
    if(cats.length){
      expCharts.cat=new Chart(cc,{type:'doughnut',data:{labels:cats.map(c=>c[0]),datasets:[{data:cats.map(c=>c[1]),backgroundColor:cats.map(c=>EXP_CATS[c[0]]||'#758BA5'),borderColor:'#0D2237',borderWidth:1.5,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'65%',plugins:{legend:{display:false}}}});
...
function renderOverallExpenses(){
  ...
  if(cc){
    const cats=Object.entries(byCat).sort((a,b)=>b[1]-a[1]);
    if(cats.length){
      overallExpCharts.cat=new Chart(cc,{type:'doughnut',data:{labels:cats.map(c=>c[0]),datasets:[{data:cats.map(c=>c[1]),backgroundColor:cats.map(c=>EXP_CATS[c[0]]||'#758BA5'),borderColor:'#0D2237',borderWidth:1.5,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'65%',plugins:{legend:{display:false}}}});
      const total=Object.values(byCat).reduce((s,v)=>s+v,0);
...
function renderLabsExpenses(){
  ...
  if(cc){
    const cats=Object.entries(byCat).sort((a,b)=>b[1]-a[1]);
    if(cats.length){
      labsExpCharts.cat=new Chart(cc,{type:'doughnut',data:{labels:cats.map(c=>c[0]),datasets:[{data:cats.map(c=>c[1]),backgroundColor:cats.map(c=>EXP_CATS[c[0]]||'#758BA5'),borderColor:'#0D2237',borderWidth:1.5,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'65%',plugins:{legend:{display:false}}}});
      const total=Object.values(byCat).reduce((s,v)=>s+v,0);
...
function renderLabsAnalytics(){
  ...
  if(lplan){
    ...
    labsCharts.plan=new Chart(lplan,{type:'doughnut',data:{datasets:[{data:[ms,ys,ls],backgroundColor:['#22C55E','#F59E0B','#8B5CF6'],borderColor:'#0D2237',borderWidth:1.5,hoverOffset:5}]},options:{responsive:true,maintainAspectRatio:false,cutout:'68%',plugins:{legend:{display:false}}}});
