...
async function markUnpaid(id){try{await apiFetch('/api/documents/'+id,{method:'PUT',body:JSON.stringify({status:'Pending'})});const idx=DB.documents.findIndex(d=>d.id===id);if(idx>=0)DB.documents[idx].status='Pending';renderDocs();renderDocStats();updateNavBadges();showToast('Invoice marked as Pending','info');}catch(e){showToast(e.message,'error');}}
