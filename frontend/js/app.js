...
async function markPaid(id){try{const d=DB.documents.find(x=>x.id===id);if(!d)return;await apiFetch('/api/documents/'+id,{method:'PUT',body:JSON.stringify({status:'Paid',paidAmount:d.total})});const idx=DB.documents.findIndex(x=>x.id===id);if(idx>=0){DB.documents[idx].status='Paid';DB.documents[idx].paidAmount=d.total;}renderDocs();renderDocStats();updateNavBadges();showToast('Invoice marked as Paid','success');}catch(e){showToast(e.message,'error');}}
async function recordPayment(id){const d=DB.documents.find(x=>x.id===id);if(!d)return;const remain=Math.max(d.total-(d.paidAmount||0),0);const raw=prompt(`Enter payment amount (remaining ${fmtFull(remain,d.currency)})`,'');if(raw===null)return;const amt=parseFloat(String(raw).replace(/[^0-9.]/g,''));if(!Number.isFinite(amt)||amt<=0){showToast('Invalid amount','error');return;}const paidNow=Math.min((d.paidAmount||0)+amt,d.total);const status=paidNow>=d.total?'Paid':'Partially Paid';try{await apiFetch('/api/documents/'+id,{method:'PUT',body:JSON.stringify({status,paidAmount:paidNow})});const idx=DB.documents.findIndex(x=>x.id===id);if(idx>=0){DB.documents[idx].status=status;DB.documents[idx].paidAmount=paidNow;}renderDocs();renderDocStats();updateNavBadges();showToast(status==='Paid'?'Invoice marked as Paid':'Partial payment recorded','success');}catch(e){showToast(e.message,'error');}}
async function markUnpaid(id){try{await apiFetch('/api/documents/'+id,{method:'PUT',body:JSON.stringify({status:'Pending',paidAmount:0})});const idx=DB.documents.findIndex(d=>d.id===id);if(idx>=0){DB.documents[idx].status='Pending';DB.documents[idx].paidAmount=0;}renderDocs();renderDocStats();updateNavBadges();showToast('Invoice marked as Pending','info');}catch(e){showToast(e.message,'error');}}
